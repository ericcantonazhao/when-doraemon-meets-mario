...
public class Tile extends Sprite
{
	...
	public static void Move(DoraemonWorld mv)
	{
		if(mv.getMario().hp == 0) return;

		if(mv.getMario().getState().equals("runright"))
		{
	...
	public void ChangeImage()
	{
		...
		case 21:
			if(this.value > 0)
			{
				this.image = LoadResourceOfDore.tile.get(index);
				this.IsTimeOver();
				if(index > 11 ) index = 8;
			}
			else
			{
				this.image = LoadResourceOfDore.tile.get(7);
			}
		break;
		...
		case 37:
			if(this.value > 0) this.image = LoadResourceOfDore.tile.get(6);
			else this.image = LoadResourceOfDore.tile.get(7);
		break;
